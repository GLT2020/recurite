<template>
  <div>
    ...
    <el-table v-loading="loading" :data="showDate" stripe style="width: 100%">
      ...
      <el-table-column v-else prop="worker.name" label="收件人" width="180">
      </el-table-column>
    ...
    <el-dialog title="具体内容" :visible.sync="dialogMsgDetails" width="50%">
      <div style="width: 90%">
        ...
        <div>
          <label>
            <span class="dialogTop">收件人:</span>
            <span>{{ msgDetailsDate.worker.name }}</span>
          </label>
        </div>
        <!-- 时间 -->
        <div>
        ...
        <div class="Msgcontent">
          ...
          <div
            ...
            @click="SiteDetails(msgDetailsDate.siteId)"
          >
            <div>
  ...
  methods: {
    ...
    openDetailsDialog(item) {
      ...
      if (this.showFlag == 0 && item.state == 0) {
        //需要将邮件设置为已读
        this.putRequest("/workermailbox/company/checkMsg", item);
        this.receiveHandle("page");
      }
    },
    ...
    deleteHandle(item) {
      if (this.showFlag == 0) {
        //收信箱删除
        this.deleteRequest("/workermailbox/deleteReceivedMsg", item).then(
          (resp) => {
            if (resp) {
              this.receiveHandle("page");
            }
        ...
        //发送箱删除
        this.deleteRequest("/companymailbox/deleteSendedMsg", item).then(
          (resp) => {
            if (resp) {
              this.sendHandle("page");
            }
    ...
    openSendHandle() {
      ...
      this.sendMsgDate.workertypeId = this.msgDetailsDate.workertypeId;

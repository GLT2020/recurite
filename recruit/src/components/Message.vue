<template>
  <div>
    ...
    <el-table :data="showDate" stripe style="width: 100%">
      <el-table-column prop="worker.name" label="收件人" width="180">
      </el-table-column>
      ...
      <el-table-column prop="state" label="状态" width="180">
        <template scope="scope">
          <el-tag v-if="scope.row.state == 0">未被查看</el-tag>
          <el-tag v-else>已查看</el-tag>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    deleteHandle(item) {
      if (this.showFlag == 0) {
        ...
        //发送箱删除
        this.deleteRequest("/companymailbox/deleteMsg/" + item.id).then(
          (resp) => {
            if (resp) {
              this.sendHandle("page");
            }
    ...
    openSendHandle() {
      ...
      this.sendMsgDate.recuritId = this.msgDetailsDate.recuritId;
